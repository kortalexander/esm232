...
lotv_wrapper = function(currpop, days, rprey, alpha, eff, pmort, K, hunted, min_pop) {
  ...
  if(result_df[length(result_df$time), 2] > 50)
    stable = TRUE
  else
    stable = FALSE
  return(stable)
...
pars = list(rprey=0.95, alpha=0.01, eff=0.6,pmort=0.4, K=2000, hunted = seq(from=100, to=300, by=10), min_pop = 300)
all_res = as.data.frame(pars) %>% pmap(lotv_wrapper, currpop = currpop, days = days)
```

We defined a prey population as stable if it remains above x number after 200 days.
